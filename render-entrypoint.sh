...
if [ -n "${WHATSAPP_HOOK_URL:-}" ]; then
  ...
  export WHATSAPP_HOOK_EVENTS="${WHATSAPP_HOOK_EVENTS:-message,message.revoked,session.status,group.v2.participants,group.v2.join,group.v2.leave,group.v2.update,poll.vote,poll.vote.failed}"
