...
if [ -n "${WHATSAPP_HOOK_URL:-}" ]; then
  ...
  export WHATSAPP_HOOK_EVENTS="${WHATSAPP_HOOK_EVENTS:-message.any,message.revoked,session.status,group.v2.participants,group.v2.join,group.v2.leave,group.v2.update,poll.vote,poll.vote.failed}"
elif [ -n "${PUBLIC_BASE_URL:-}" ]; then
  ENCODED_WEBHOOK_SECRET="$(node -e 'console.log(encodeURIComponent(process.env.WEBHOOK_SECRET || "change-me"))')"
  export WHATSAPP_HOOK_URL="${PUBLIC_BASE_URL%/}/webhooks/waha?secret=${ENCODED_WEBHOOK_SECRET}"
  export WHATSAPP_HOOK_EVENTS="${WHATSAPP_HOOK_EVENTS:-message.any,message.revoked,session.status,group.v2.participants,group.v2.join,group.v2.leave,group.v2.update,poll.vote,poll.vote.failed}"
fi
