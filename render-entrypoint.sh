...
# Give WAHA time to open its HTTP port before the bot starts. Render's free
# tier alone can add 50+ seconds of cold-start delay, and WAHA's own startup
# (DB migrations, GOWS engine init) can take a while beyond that — 30s was
# cutting it so close that WAHA would finish starting (see its own "Nest
# application successfully started" log line) within a second of this loop
# giving up, get killed anyway, and force a full container restart loop.
# WAHA_READY_TIMEOUT_S lets you raise this further if your instance is slower
# still (e.g. after a long spin-down on the free plan).
...
while [ "$i" -lt "$READY_TIMEOUT" ]; do
  ...
  i=$((i + 1))
  if [ "$((i % 15))" -eq 0 ]; then
    echo "[render] Still waiting for WAHA to become ready (${i}s elapsed, timeout ${READY_TIMEOUT}s)..."
  fi
  sleep 1
done
