...
export async function runSplitGcJob(session, sourceGroupId, job, options = {}) {
  ...
  try {
    ...
    if (!job.newId) {
      ...
      const created = await createGroup(session, job.subject, first);
      ...
      try { await setGroupSubject(session, newId, job.subject); } catch {}
      if (job.description) { try { await setGroupDescription(session, newId, job.description); } catch {} }
