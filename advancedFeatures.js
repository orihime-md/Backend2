...
export async function cloneGroup(session, sourceGroupId, sourceGroup, participants, botIds = []) {
  ...
  const description = String(sourceGroup?.description || '').slice(0, 512);

  const first = memberIds.slice(0, 20).map((id) => ({ id }));
  ...
  for (let i = 20; i < memberIds.length; i += 20) {
    const chunk = memberIds.slice(i, i + 20).map((id) => ({ id }));
    try { await addParticipants(session, newId, chunk); } catch (error) { console.error('[split-gc] add chunk failed:', error?.response?.data || error?.message || error); }
  }
  ...
  try {
    const picture = await getGroupPicture(session, sourceGroupId);
    if (picture?.url) await setGroupPicture(session, newId, { url: picture.url });
  } catch (error) {
    console.error('[split-gc] picture copy failed:', error?.response?.data || error?.message || error);
  }
  ...
  for (const [name, field] of security) {
    try {
      const val = await getGroupSecurity(session, sourceGroupId, name);
      if (val?.[field] !== undefined) await setGroupSecurity(session, newId, name, { [field]: Boolean(val[field]) });
    } catch {}
  }

  if (uniqueAdmins.length) {
    for (let i = 0; i < uniqueAdmins.length; i += 20) {
      const chunk = uniqueAdmins.slice(i, i + 20).map((id) => ({ id }));
