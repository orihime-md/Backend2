...
export function buildWebhookConfig() {
  ...
  return {
    webhooks: [
      {
        url: webhookUrl,
        events: ['message', 'message.any', 'message.revoked', 'session.status', 'group.v2.participants', 'group.v2.join', 'group.v2.leave']
      }
    ]
  };
}
...
export async function createSession(name) {
  return request('post', '/api/sessions', {
    data: { name, config: buildWebhookConfig() }
  });
}
...
// WAHA converts whatever image/video you hand it into a webp sticker
// server-side, so this container doesn't need its own image-processing
// dependencies (sharp/ffmpeg) just for .sticker.
export async function sendSticker(name, chatId, file) {
  return request('post', '/api/sendImage', {
    data: { session: name, chatId, file, asSticker: true },
    timeout: 120000
  }).catch(() =>
    request('post', '/api/sendSticker', {
