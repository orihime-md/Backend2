...
// `paging` can be a plain number (legacy: just a limit, offset 0) or
// { limit, offset } for walking backward through the channel's full history
// page by page — offset is how the deep backfill reaches messages posted
// long before this session ever followed the channel.
function normalizePaging(paging) {
  if (typeof paging === 'number') return { limit: paging, offset: 0 };
  const { limit = 100, offset = 0 } = paging || {};
  return { limit, offset };
}

export async function getChannelMessages(name, channelId, paging = 100) {
  const { limit, offset } = normalizePaging(paging);
  return request('get', `/api/${encodeURIComponent(name)}/chats/${encodeURIComponent(channelId)}/messages?downloadMedia=true&limit=${limit}&offset=${offset}`, { timeout: 60000 });
}

export async function getChannelPreview(name, ref, paging = 100) {
  const { limit, offset } = normalizePaging(paging);
  return request('get', `/api/${encodeURIComponent(name)}/channels/${encodeURIComponent(ref)}/messages/preview?downloadMedia=true&limit=${limit}&offset=${offset}`, { timeout: 60000 });
}
