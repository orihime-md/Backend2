...
export function buildWebhookConfig() {
  ...
  return {
    webhooks: [
      {
        url: webhookUrl,
        // 'message' only covers messages WAHA received from someone else.
        // Messages typed from the linked/paired number itself (fromMe: true —
        // e.g. the owner typing commands directly on the phone the bot is
        // running as) are NOT delivered under 'message' at all — only under
        // the separate 'message.any' event. Without it those commands never
        // even reach the webhook, so the linked number silently gets no
        // response while everyone else does. 'message.any' is a superset of
        // 'message' (same payload shape, fires for received AND self-sent
        // messages), so it fully replaces 'message' here rather than adding
        // to it — subscribing to both would double-fire on every normal
        // incoming message and cause duplicate replies.
        events: ['message.any', 'message.revoked', 'session.status', 'group.v2.participants', 'group.v2.join', 'group.v2.leave', 'group.v2.update', 'poll.vote', 'poll.vote.failed']
      }
    ]
  };
}
...
// Resolves a bare phone number (e.g. BOT_OWNER_PHONE) to WhatsApp's own
// identifiers for it: the phone-based chatId and, where the running engine
// tracks one, the hidden @lid twin. This is what lets a phone number typed
// into an env var be matched directly against a group message's sender id
// later, instead of only ever comparing raw digits.
export async function checkNumberStatus(name, phone) {
  const digits = String(phone || '').replace(/\D/g, '');
  if (!digits) return null;
  return request('get', '/api/contacts/check-exists', {
    params: { session: name, phone: digits },
    timeout: 20000
  });
}

// WAHA's contact record for an id (phone JID or @lid). Some engines only
// surface the phone<->lid pairing here rather than on check-exists, so this
// is tried as a second source when the first didn't return a lid.
export async function getContact(name, contactId) {
  return request('get', '/api/contacts', {
    params: { session: name, contactId },
    timeout: 20000
  });
}
