...
export async function syncChannelImages(session, { deep = false } = {}) {
  ...
  if (!deep) {
    ...
    for (let page = 0; page < SYNC_MAX_PAGES; page += 1) {
      ...
      // If offset isn't actually honored by this WAHA version, every "page"
      // comes back identical — walking 300 pages of the same 100 messages
      // would look like progress but never reach anything older. Detect
      // that and stop immediately instead of burning through SYNC_MAX_PAGES
      // for nothing.
      const ids = list.map((m) => m.id || m._id || '').join(',');
      if (previousIds && ids === previousIds) {
        stoppedReason = 'offset-not-honored';
        console.warn(
          `[channel-sync] offset param had no effect on page ${page} for ${session} — ` +
          'this WAHA build/engine may not support paging channel history past its first page. ' +
          'Only the most recent window of messages can be indexed.'
        );
        break;
      }
      previousIds = ids;
      totalCandidates += list.length;
      synced += await processMessages(list, session);
      if (list.length < SYNC_PAGE_SIZE) {
        stoppedReason = 'short-page';
        break; // short page = reached the oldest post
      }
    }
    if (!stoppedReason) stoppedReason = 'max-pages';
    console.log(`[channel-sync] deep backfill for ${session}: ${pages} page(s), ${totalCandidates} message(s) scanned, ${synced} media/link item(s) saved, stopped: ${stoppedReason}`);
    // Only mark the deep pass complete if it actually reached the end of
    // history (or ran out of headroom) — NOT if offset turned out to be
    // unsupported, so that case is visible via the warning above every time
    // rather than being silently marked "done" after indexing one page.
    if (stoppedReason !== 'offset-not-honored') {
  ...
  return { ok: true, synced, channelId, totalCandidates, pages, deep, stoppedReason };
}
    ...
    for (const s of sessions) {
      ...
      try {
        if (!deepSyncStarted.has(name) && !getSession(name)?.channelDeepSyncCompletedAt) {
          ...
          // history walk, which can take a while on a large channel. If it
          // throws, un-block it so the NEXT tick retries instead of never
          // trying again for the rest of this process's lifetime.
          syncChannelImages(name, { deep: true }).catch((e) => {
            deepSyncStarted.delete(name);
            console.error('[channel-watcher] deep backfill failed, will retry next tick:', name, e?.response?.data || e?.message || e);
          });
        }
