...
import { ensureChannelFollow } from './channelSync.js';
  ...
  for (const remote of apiSessions) {
    ...
    if (status === 'WORKING') {
      ...
      if (!pending.has(name)) {
        pending.add(name);
        try {
          // Just make sure the session follows the channel. The actual
          // media scanning (including the one-time full-history backfill)
          // is owned by channelSync.js's own background watcher now, so it
          // isn't tied to this 20s reconcile loop.
