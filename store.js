...
function cleanLoadedState(value) {
  ...
  // Owner/admin startup scanning was removed. Drop all legacy scan state so
  // old deployments stop carrying group-wide admin snapshots forward.
  for (const session of Object.values(next.sessions || {})) {
    delete session.ownerAdminGroups;
    delete session.ownerAdminScan;
    delete session.ownerIds;
  }
  delete next.links;
  return next;
}
...
export async function upsertSession(name, patch = {}) {
  ...
  delete cleanPatch.ownerAdminGroups;
  delete cleanPatch.ownerAdminScan;
  delete cleanPatch.ownerIds;
  state.sessions[name] = {
    ...(state.sessions[name] || {
      name,
