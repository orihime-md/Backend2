  ...
  if (cached) {
    ...
    if (!message) {
      const hint = error === 'no-media'
        ...
        : `Could not read that message (${error}). WhatsApp/WAHA may already have consumed the view-once payload, or the current engine may not have exposed its media. Reply with .vv as soon as the view-once arrives.`;
...
const VIEWONCE_CACHE_TTL_MS = 30 * 60 * 1000;
    ...
    case 'play': {
      ...
      if (rapidApiConfigured()) {
        try {
          ...
          const downloaded = await rapidYoutubeDownload(videoId);
          ...
          rapidError = error;
          console.error('[bot] RapidAPI play failed:', error?.message || error);
        }
      }
