...
async function executeCommand({ session, chatId, from, parsed, payload, isOwner }) {
  ...
  switch (name) {
    ...
    case 'split-gc': {
      ...
      await sendCommandReply(session, chatId, '.split-gc', `🪞 Creating a new group and adding members ${SPLIT_GC_BATCH_SIZE || 5} at a time (paced, to avoid a WhatsApp spam flag). This can take a while for a large group — I'll post progress here.`);
