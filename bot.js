...
async function getGroupAdminStatus(session, chatId, senderJid, payload, { live = false } = {}) {
  ...
  const ownerIds = new Set([ownerJid].filter(Boolean));
  for (const id of stored?.ownerIds || []) if (id) ownerIds.add(normalizeJid(id) || id);
  const isSelf = Boolean(payload?.fromMe) || senderIds.some((id) => ownerIds.has(id));
  const ownerAdminRecord = stored?.ownerAdminGroups?.[chatId] || null;

  // Hot path: permission checks never enumerate the group. They compare only
  // the message sender with the saved linked-owner identifiers and the saved
  // owner admin bit for this group. `live:true` is reserved for diagnostics
  // such as .whoami/.listadmin.
  if (!live) {
    const senderEntry = isSelf ? {
      ids: [...ownerIds],
      phoneJid: ownerJid || '',
      lidJid: '',
      role: ownerAdminRecord?.isAdmin ? 'admin' : 'member',
      isAdmin: ownerAdminRecord?.isAdmin === true,
      isSuperAdmin: false,
      isMe: true,
      raw: { id: ownerJid }
    } : null;
    const botEntry = ownerAdminRecord ? {
      ids: Array.isArray(ownerAdminRecord.ownerIds) && ownerAdminRecord.ownerIds.length ? ownerAdminRecord.ownerIds : [...ownerIds],
      phoneJid: ownerJid || '',
      lidJid: '',
      role: ownerAdminRecord.isAdmin ? 'admin' : 'member',
      isAdmin: ownerAdminRecord.isAdmin === true,
      isSuperAdmin: ownerAdminRecord.isSuperAdmin === true,
      isMe: true,
      raw: { id: ownerJid }
    } : null;
    return {
      ...
      senderEntry,
      botEntry,
      senderIsAdmin: Boolean(isSelf && ownerAdminRecord?.isAdmin === true),
      botIsAdmin: Boolean(ownerAdminRecord?.isAdmin === true),
      ownerJid,
      ownerAdminRecord
    };
  }

  // Diagnostic/live path retained for .whoami and .listadmin only.
  const selfIds = await getSelfIds(session);
...
async function assertGroupAdmin(session, chatId, senderJid, payload) {
  ...
  const status = await getGroupAdminStatus(session, chatId, senderJid, payload, { live: false });
  if (!status.isSelf) {
    return {
      ok: false,
      repel: true,
      message: '⛔ Only the linked owner number saved for this group can use admin features.'
    };
  }
  if (!status.ownerAdminRecord) {
    return {
      ok: false,
      message: '⏳ Owner admin status is still being scanned for this group. Please retry shortly.'
    };
  }
  ...
  return {
    ...
    ownerAdminRecord: status.ownerAdminRecord,
    participants: null,
    ...status
  };
}
...
async function withOwnerScanTimeout(taskFactory, timeoutMs) {
  ...
  try {
    ...
    if (error?.name === 'CanceledError' || error?.code === 'ERR_CANCELED' || controller.signal.aborted) {
      throw new Error(`owner admin scan timed out after ${ms}ms`);
    }
    throw error;
  } finally {
...
async function scanOwnerAdminGroup(session, groupId, groupMeta = {}, ownerJid, ownerIds) {
  const record = {
    groupId,
    groupName: groupNameFromValue(groupMeta, groupId),
    ownerJid,
    ownerIds: [...ownerIds],
    isMember: null,
    isAdmin: null,
  ...
  try {
    // WAHA exposes only participant-list endpoints for group membership. We
    // request ONE group at a time, search only for the linked owner's IDs, then
    // immediately discard the participant array. No member count is calculated
    // and no participant list is persisted.
    const raw = await withOwnerScanTimeout(
      (signal) => getParticipantsV2(session, groupId, {
        timeout: Number(process.env.OWNER_ADMIN_SCAN_TIMEOUT_MS || 8000),
        signal
      }),
      process.env.OWNER_ADMIN_SCAN_TIMEOUT_MS || 8000
    );
    const entries = extractParticipantList(raw).map(parseParticipant);
    const entry = findEntry(entries, [...ownerIds], { isSelf: true });
    record.isMember = Boolean(entry);
    record.isAdmin = Boolean(entry?.isAdmin);
    record.isSuperAdmin = Boolean(entry?.isSuperAdmin);
    if (entry?.ids?.length) record.ownerIds = entry.ids;
  } catch (error) {
    record.error = String(error?.response?.data?.message || error?.message || error).slice(0, 240);
    console.error('[owner-admin-scan] group lookup failed:', { session, groupId, error: record.error });
  }

  return record;
}

const OWNER_ADMIN_SCAN_IN_FLIGHT = new Map();

export async function scanOwnerAdminGroups(session, { reason = 'working' } = {}) {
  if (!session) return null;
  const enabled = String(process.env.OWNER_ADMIN_SCAN_ENABLED ?? 'true').toLowerCase() !== 'false';
  if (!enabled) {
    console.log('[owner-admin-scan] disabled by OWNER_ADMIN_SCAN_ENABLED', { session, reason });
    return null;
  }
  const running = OWNER_ADMIN_SCAN_IN_FLIGHT.get(session);
  if (running) return running;

  const task = (async () => {
    const startedAt = Date.now();
    let ownerJid = getStored(session)?.ownerJid || '';
    if (!ownerJid) ownerJid = await getBotOwner(session);
    if (!ownerJid) {
      console.warn('[owner-admin-scan] skipped — linked owner number is not known', { session });
      return null;
    }

    const ownerIds = new Set([ownerJid].filter(Boolean));
    try { for (const id of await getSelfIds(session)) if (id) ownerIds.add(id); } catch {}
    await upsertSession(session, { ownerJid, ownerIds: [...ownerIds] });

    const pageSize = Math.max(1, Math.min(10, Number(process.env.OWNER_ADMIN_SCAN_PAGE_SIZE || 5)));
    const delayMs = Math.max(0, Number(process.env.OWNER_ADMIN_SCAN_DELAY_MS || 350));
    const maxGroups = Math.max(1, Number(process.env.OWNER_ADMIN_SCAN_MAX_GROUPS || 100));
    const timeoutMs = Math.max(1000, Number(process.env.OWNER_ADMIN_SCAN_TIMEOUT_MS || 8000));
    let offset = 0;
    let scannedGroups = 0;
    let adminGroups = 0;
    let failedGroups = 0;
    ...
    while (scannedGroups < maxGroups) {
      let result;
      try {
        result = await withOwnerScanTimeout(
          (signal) => getGroups(session, {
            limit: pageSize,
            offset,
            sortBy: 'subject',
            sortOrder: 'asc',
        ...
        console.error('[owner-admin-scan] group list failed:', { session, offset, error: error?.response?.data || error?.message || String(error) });
        break;
      }

      const groups = extractGroupsList(result);
      if (!groups.length) break;

      for (const group of groups) {
        if (scannedGroups >= maxGroups) break;
        const groupId = groupIdFromValue(group);
        if (!isGroup(groupId)) continue;
        const snapshot = await scanOwnerAdminGroup(session, groupId, group, ownerJid, ownerIds);
        snapshots[groupId] = snapshot;
        scannedGroups += 1;
        if (snapshot.isAdmin === true) adminGroups += 1;
        if (snapshot.error) failedGroups += 1;
    ...
    await upsertSession(session, {
      ownerJid,
      ownerIds: [...ownerIds],
      ownerAdminGroups: snapshots,
      ownerAdminScan: scan
    });
    console.log('[owner-admin-scan] complete', { session, scannedGroups, adminGroups, failedGroups, durationMs: scan.durationMs });
    return scan;
  })().catch((error) => {
    console.error('[owner-admin-scan] fatal error:', { session, error: error?.message || String(error) });
    return null;
  }).finally(() => OWNER_ADMIN_SCAN_IN_FLIGHT.delete(session));

  OWNER_ADMIN_SCAN_IN_FLIGHT.set(session, task);
  return task;
}

async function updateOwnerAdminSnapshotFromGroupEvent(session, eventName, payload) {
  const groupId = groupIdFromValue(payload?.group || payload);
  if (!isGroup(groupId)) return;
  const stored = getStored(session);
  const ownerJid = stored?.ownerJid;
  if (!ownerJid) return;

  const ownerIds = new Set([ownerJid, ...(Array.isArray(stored?.ownerIds) ? stored.ownerIds : [])].filter(Boolean));
  let changed = false;
  const current = { ...(stored?.ownerAdminGroups?.[groupId] || {
    groupId,
    groupName: groupNameFromValue(payload?.group, groupId),
    ownerJid,
    ownerIds: [...ownerIds]
  }) };
  ...
  if (eventName === 'group.v2.join') {
    ...
    const entries = groupParticipantsFromPayload(payload?.group || payload).map(parseParticipant);
    const ownerEntry = findEntry(entries, [...ownerIds], { isSelf: true });
    if (ownerEntry) {
      current.isAdmin = Boolean(ownerEntry.isAdmin);
      current.isSuperAdmin = Boolean(ownerEntry.isSuperAdmin);
      current.ownerIds = ownerEntry.ids;
    }
    changed = true;
  } else if (eventName === 'group.v2.leave') {
    current.isMember = false;
    current.isAdmin = false;
    changed = true;
  } else if (eventName === 'group.v2.participants') {
    const type = String(payload?.type || payload?.action || '').toLowerCase();
    const entries = groupParticipantsFromPayload(payload).map(parseParticipant);
    const ownerEntry = entries.find((entry) => entry.ids.some((id) => ownerIds.has(id)) || entry.isMe);
    if (ownerEntry) {
      current.isMember = type === 'leave' ? false : true;
      current.isAdmin = type === 'leave' ? false : Boolean(ownerEntry.isAdmin);
      current.isSuperAdmin = type === 'leave' ? false : Boolean(ownerEntry.isSuperAdmin);
      if (ownerEntry.ids?.length) current.ownerIds = ownerEntry.ids;
      changed = true;
    }
  }
  ...
  current.source = 'group-event';
  const nextGroups = { ...(stored?.ownerAdminGroups || {}), [groupId]: current };
  await upsertSession(session, { ownerAdminGroups: nextGroups, ownerIds: [...ownerIds] });
}

async function handleGroupEventInternal(session, eventName, payload) {
  const groupId = payload?.group?.id || payload?.id;
  if (!isGroup(groupId)) return;
  try { await updateOwnerAdminSnapshotFromGroupEvent(session, eventName, payload); } catch (error) {
    console.error('[owner-admin-scan] group event update failed:', error?.message || error);
  }
