...
export async function handleWebhookEvent(session, payload) {
  ...
  if (mode === 'PRIVATE' && !isOwner && !ALWAYS_PUBLIC_COMMANDS.has(parsed.name)) {
  ...
  switch (name) {
    ...
    case 'antipromote': {
      ...
      await genericStatus(session, chatId, name, [
        ...
        action === 'on' ? explain : '',
        `👤 Requested by : ${senderMention}`
      ].filter(Boolean), { imageKey: '.menu', title: `🛡️ ${name.toUpperCase()}` });
      return;
    }
    ...
    case 'link': {
      ...
      await genericStatus(session, chatId, name, lines, { title: '🔗 𝐋𝐈𝐍𝐊𝐒', imageKey: '.menu' });
      return;
    }
    ...
    case 'autod': {
      ...
      ], { title: '🗑️ 𝐀𝐔𝐓𝐎 𝐃𝐄𝐋𝐄𝐓𝐄', imageKey: '.menu' });
      return;
    }
    ...
    case 'listadmin': {
      ...
      await genericStatus(session, chatId, name, lines, { title: '👑 𝐀𝐃𝐌𝐈𝐍 𝐋𝐈𝐒𝐓', imageKey: '.menu' });
      return;
    }
