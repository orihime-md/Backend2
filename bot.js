...
async function withOwnerScanTimeout(promise, timeoutMs) {
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`owner admin scan timed out after ${ms}ms`)), ms);
      })
    ]);
  } finally {
...
async function scanOwnerAdminGroup(session, groupId, groupMeta = {}, ownerJid, ownerIds) {
  ...
  try {
    ...
    const raw = await withOwnerScanTimeout(getParticipantsV2(session, groupId), process.env.OWNER_ADMIN_SCAN_TIMEOUT_MS || 15000);
...
export async function scanOwnerAdminGroups(session, { reason = 'working' } = {}) {
  ...
  const task = (async () => {
    ...
    const pageSize = Math.max(1, Math.min(50, Number(process.env.OWNER_ADMIN_SCAN_PAGE_SIZE || 20)));
    const delayMs = Math.max(0, Number(process.env.OWNER_ADMIN_SCAN_DELAY_MS || 120));
    let offset = 0;
    let scannedGroups = 0;
    let adminGroups = 0;
    let failedGroups = 0;
    ...
    while (true) {
      let result;
      try {
        result = await withOwnerScanTimeout(getGroups(session, {
          limit: pageSize,
          offset,
          sortBy: 'id',
          sortOrder: 'asc',
          exclude: 'participants'
        }), process.env.OWNER_ADMIN_SCAN_TIMEOUT_MS || 15000);
      } catch (error) {
