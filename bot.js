...
async function handleWebhookEventInternal(session, payload) {
  ...
  if (!NO_INTRO_COMMANDS.has(parsed.name)) {
    // Never let a failed intro message swallow the actual command reply.
    try { await sendIntro(session, chatId); }
    catch (error) { console.error('[bot] intro send failed:', error?.response?.data || error?.message || error); }
  }
