...
async function revealViewOnce(session, chatId, payload) {
  const selfIds = await getSelfIds(session);
  const candidates = quotedMessageIdCandidates(payload, chatId, selfIds);
  if (!candidates.length) return sendText(session, chatId, '❌ Reply to a view-once message with .vv');
  ...
  if (cached) {
    ...
    if (!message) {
      const hint = error === 'no-media'
        ...
        : `Could not read that message (${error}). It was likely already opened on the linked phone before .vv was sent, which makes WhatsApp discard the view-once content everywhere.`;
      return sendText(session, chatId, `❌ ${hint}`);
    }
    wasViewOnce = Boolean(message.isViewOnce) || /viewonce/i.test(JSON.stringify(message._data || {}));
    const media = await downloadMedia(message.media.url); // in-memory only
  ...
  try {
    ...
    const file = {
      mimetype,
      filename: filename || `viewonce.${ext}`,
    ...
    buffer = null; // drop the buffer; nothing was persisted
  }
}
...
async function applyAdminAction(session, groupId, action, target) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    await sleep(500);
    invalidateParticipantsCache(session, groupId); // this loop needs a true fresh read each attempt, not the cached snapshot
    try {
...
async function handleParticipantsEvent(session, groupId, payload) {
  ...
  // The webhook and getSelfIds() can report the same person under different
  // JID formats (phone @c.us vs privacy @lid), so a plain selfIds.includes()
  // can miss and let the bot "correct" its own linked account / owner.
  // Resolve the live participant list once and match on the full id set
  // (and the engine's own isMe flag) so every format is covered.
  let selfCheckEntries = [];
  try {
    selfCheckEntries = (await resolveParticipants(session, groupId, { deep: true })).map(parseParticipant);
  } catch {}
  const isBotLinkedUser = (target) => {
    if (selfIds.includes(target)) return true;
    const entry = selfCheckEntries.find((e) => e.ids.includes(target));
    return Boolean(entry && (entry.isMe || entry.ids.some((id) => selfIds.includes(id))));
  };
...
const VIEWONCE_CACHE_MAX = 40;
const VIEWONCE_CACHE_TTL_MS = 15 * 60 * 1000;
...
async function precacheViewOnceMedia(session, chatId, id, media) {
  ...
  try {
    const downloaded = await downloadMedia(media.url);
...
async function handleWebhookEventInternal(session, payload) {
  ...
  if (payload?.hasMedia && payload?.media?.url) {
    const isViewOnce = Boolean(payload.isViewOnce) || /viewonce/i.test(JSON.stringify(payload._data || {}));
    if (isViewOnce) {
      const msgId = payload?.id || payload?.messageId || payload?._data?.Info?.ID;
...
async function executeCommand({ session, chatId, from, parsed, payload, isOwner }) {
  ...
  switch (name) {
    ...
    case 'sticker': {
      ...
      const mimetype = String(message.media.mimetype || '').split(';')[0];
      if (!mimetype.startsWith('image/') && !mimetype.startsWith('video/')) {
        return sendText(session, chatId, '❌ Reply to an image or a short video to make a sticker.');
      }
      const media = await downloadMedia(message.media.url);
      if (!media?.buffer?.length) return sendText(session, chatId, '❌ Could not download that media.');
      const file = { mimetype, filename: `sticker.${mimetype.split('/')[1] || 'webp'}`, data: media.buffer.toString('base64') };
      try {
        await sendSticker(session, chatId, file);
      } catch (error) {
        console.error('[bot] sticker failed:', error?.response?.data || error?.message || error);
        await sendText(session, chatId, '❌ Could not create the sticker. Your WAHA version/engine may not support sticker conversion.');
      }
      return;
    }
    ...
    // Music/audio downloader. Uses play-dl (a plain-npm YouTube client, no
    // API key) lazily imported so a missing/broken install fails with a
    // clear message instead of crashing the whole bot process at startup.
    // NOTE: downloading copyrighted music is between you and the rights
    // holders/YouTube's terms — this is a generic utility, use responsibly.
    case 'play': {
      ...
      try {
        ...
        return genericStatus(session, chatId, name, [
          ...
          '⚙️ Run "npm install play-dl" and redeploy to enable .play.'
        ], { title: '🎵 𝐏𝐋𝐀𝐘' });
      }
      ...
      try {
        ...
        if (!/^https?:\/\/(www\.)?(youtube\.com|youtu\.be)\//i.test(url)) {
          ...
          if (!results?.length) return sendText(session, chatId, '❌ No results found for that song.');
          url = results[0].url;
          title = results[0].title || joined;
        }
        const stream = await playdl.stream(url, { discordPlayerCompatibility: false });
        const chunks = [];
        ...
        if (!buffer.length) return sendText(session, chatId, '❌ Could not download audio for that song.');
        if (buffer.length > 60 * 1024 * 1024) return sendText(session, chatId, '❌ That track is too large to send (over 60MB).');
        const mimetype = stream.type === 'opus' ? 'audio/ogg; codecs=opus' : 'audio/mpeg';
        const ext = stream.type === 'opus' ? 'ogg' : 'mp3';
        const file = { mimetype, filename: `${String(title).slice(0, 60).replace(/[\\/:*?"<>|]/g, '')}.${ext}`, data: buffer.toString('base64') };
        await sendFile(session, chatId, file, `🎵 ${title}`);
      } catch (error) {
        console.error('[bot] play failed:', error?.message || error);
        await genericStatus(session, chatId, name, [
          '🔴 Status : 𝐃𝐎𝐖𝐍𝐋𝐎𝐀𝐃 𝐅𝐀𝐈𝐋𝐄𝐃',
          `⚠️ ${String(error?.message || 'Could not fetch that track').slice(0, 200)}`
        ], { title: '🎵 𝐏𝐋𝐀𝐘' });
      }
      return;
    }
