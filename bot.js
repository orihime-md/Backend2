...
let interactiveSendBroken = false;

// Sends the MAIN .menu message. Unlike the category/toggle sub-menus, the
// main menu can carry a channel-synced header image, and that image needs
// to be part of the SAME bubble as the text whenever interactive sending
// isn't available (a separate image message + a separate text message reads
// as two disconnected messages, not one polished menu). So: try the
// interactive list first; if that lands, the image (if any) tags along as
// its own flourish message. If interactive is unavailable, fall back to the
// single classic image+caption (or text-only) message, exactly as before.
async function sendMenuMessage(session, chatId, prefix) {
  const listPayload = buildMainMenuPayload(prefix);
  const text = buildMenuText(prefix);

  if (!interactiveSendBroken) {
    try {
      ...
      interactiveSendBroken = true;
      const status = error?.response?.status;
      const detail = error?.response?.data || error?.message || error;
      if (status === 501) {
        console.error('[menu] interactive send: WAHA returned 501 — this engine does not implement sendList/sendButtons at all. Falling back to the classic text menu for the rest of this session.', detail);
      } else if (status === 403 || status === 402) {
        console.error('[menu] interactive send: WAHA rejected this as a Plus-tier-only feature (status ' + status + '). Falling back to the classic text menu for the rest of this session. Check GET /api/server/environment for your "tier".', detail);
      } else {
        console.error('[menu] interactive send failed, falling back to the classic text menu:', detail);
      }
    }
  }

  // Interactive unavailable (or already known to be) — one merged message,
  // same as the pre-interactive version of this bot: the channel image (if
  // any) carries the full menu text as its caption; if there's no synced
  // image yet, sendChannelImage just sends the text.
  await sendChannelImage(session, chatId, '.menu', text);
}
...
async function executeCommand({ session, chatId, from, parsed, payload, isOwner }) {
  ...
  switch (name) {
    case 'menu': {
      ...
      // "moving" part — then clear it and send the real menu.
