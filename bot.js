...
function normalizeJid(input) {
  ...
  // WhatsApp JIDs from multi-device sessions carry a ":<deviceId>" suffix
  // (e.g. "1234567890:14@s.whatsapp.net") that isn't part of the phone
  // number — stripping only non-digit characters (the old behaviour) left
  // those device digits glued onto the number, so the same account never
  // matched itself across two different payloads.
  const digits = String(userPart || '').split(':')[0].replace(/\D/g, '');
  if (!digits) return '';
  // @lid ("Linked ID") is WhatsApp's privacy identifier — a random ID unrelated
  // to the phone number, shown instead of it in groups with hidden numbers.
  // Keep it tagged as @lid rather than collapsing it into a fake @c.us JID,
  // so it's never mistaken for (or matched against) a real phone number.
...
// Newer WAHA versions fill in the real phone number on a group participant
// (as `pn`) even when its `id` is reported as an anonymous @lid — this is
// the most reliable way to recognize "me" in a group where numbers are hidden.
function participantPhoneJid(p) {
  return normalizeJid(p?.pn || p?.phoneNumber || p?.number || '');
}
...
// Matches a participant record against a target JID, tolerating the two
// common reasons a straight string comparison fails for the SAME account:
// the participant is reported as an anonymous @lid (WhatsApp's "hide my
// number" privacy setting) instead of a phone-based JID, or an engine
// exposes an explicit `isMe`/`isSelf` flag instead of a matching JID at all.
function findParticipantRecord(participants, targetJid, { isSelf = false } = {}) {
  if (!targetJid && !isSelf) return undefined;
  const targetDigits = jidNumber(targetJid);
  return participants.find((p) => {
    if (isSelf && (p?.isMe === true || p?.isSelf === true)) return true;
    const pJid = participantJid(p);
    if (pJid && targetJid && pJid === targetJid) return true;
    const phoneJid = participantPhoneJid(p);
    if (phoneJid && targetJid && phoneJid === targetJid) return true;
    if (targetDigits) {
      if (pJid && !pJid.endsWith('@lid') && jidNumber(pJid) === targetDigits) return true;
      if (phoneJid && jidNumber(phoneJid) === targetDigits) return true;
    }
    return false;
  });
}
...
async function assertGroupAdmin(session, chatId, senderJid) {
  ...
  const isSelf = Boolean(me && sender === me);
  let senderRecord = findParticipantRecord(participants, sender, { isSelf });
  let botRecord = findParticipantRecord(participants, me, { isSelf: true });
  // When the command sender IS the linked account (the owner typing their own
  // commands), sender and bot are provably the same WhatsApp account — reuse
  // whichever record matched, rather than requiring two independent JID
  // lookups to both succeed despite @lid/device-suffix formatting quirks.
  if (isSelf) {
    senderRecord = senderRecord || botRecord;
    botRecord = botRecord || senderRecord;
    if (!senderRecord) {
      console.warn('[bot] could not match the linked account against group participants', { chatId, me, sender, participantCount: participants.length });
    }
  }
...
async function isSenderGroupAdmin(session, chatId, senderJid) {
  try {
    ...
    const isSelf = Boolean(owner && sender === owner);
    const senderRecord = findParticipantRecord(participants, sender, { isSelf });
...
export async function handleWebhookEvent(session, payload) {
  ...
  const from = payload.fromMe
    ? (owner || normalizeJid(payload.from || payload.sender || payload.author || payload._data?.Info?.Sender))
    : normalizeJid(payload.from || payload.sender || payload.author || payload._data?.Info?.Sender);
...
async function executeCommand({ session, chatId, from, parsed, payload, isOwner }) {
  ...
  switch (name) {
    ...
    case 'whoami': {
      ...
      // point is to see why that check does or doesn't pass. Shows exactly
      // what JID the bot resolved for you and for itself, and (in a group)
      // how each matches against the raw participant list, so a mismatch
      // caused by @lid or a device-suffix JID is visible instead of silent.
      const me = normalizeJid((await getMe(session))?.id || '');
      ...
      if (isGroup(chatId)) {
        try {
          const participants = await resolveParticipants(session, chatId);
          const describe = (p) => {
            if (!p) return 'NOT FOUND in participant list';
            const phone = participantPhoneJid(p);
            const role = participantRole(p) || (p?.isAdmin || p?.isSuperAdmin ? 'admin' : 'member');
            return `${participantJid(p) || '(no id)'}${phone ? ` / pn:${phone}` : ''} — role=${role}`;
          };
          const senderRecord = findParticipantRecord(participants, sender, { isSelf: sameAccount });
          const botRecord = findParticipantRecord(participants, me, { isSelf: true });
          lines.push(
            `👥 Participants loaded : ${participants.length}`,
            `👤 Your record : ${describe(senderRecord)}`,
            `🤖 Bot record : ${describe(botRecord)}`
          );
          const sample = participants.slice(0, 8).map((p) => `• ${participantJid(p) || '?'}${participantPhoneJid(p) ? ` (pn:${participantPhoneJid(p)})` : ''} — ${participantRole(p) || (p?.isAdmin || p?.isSuperAdmin ? 'admin' : 'member')}`);
          if (sample.length) lines.push('📋 First 8 raw entries :', ...sample);
        } catch (error) {
      ...
      await genericStatus(session, chatId, name, lines, { title: '🕵️ 𝐖𝐇𝐎𝐀𝐌𝐈', imageKey: '.menu' });
      return;
    }
