...
// Maps the built-in .lang-<key> ciphers to real language names for AI
// translation of the LINKED NUMBER's own (fromMe) messages. Keys not listed
// here (custom .lang-add ciphers) aren't real languages, so those keep using
// straight alphabet substitution instead — see translateAndEdit's call site.
const LANGUAGE_AI_NAMES = { china: 'Chinese', japanese: 'Japanese', korea: 'Korean' };

// Translates `body` to `aiLanguageName` and edits the message in place.
// Deliberately minimal — one Gemini call, one WAHA edit call, nothing else —
// since this fires on every single message the linked number sends while a
// language mode is on, and any extra step here is extra latency on THEIR
// own typing.
async function translateAndEdit(session, chatId, messageId, body, aiLanguageName) {
  const prompt = `Translate the following message to ${aiLanguageName}. Reply with ONLY the translation, nothing else — no notes, no quotes, no explanation:\n\n${body.slice(0, 4000)}`;
  const translated = await askGemini(prompt);
  if (translated && translated.trim()) {
    await editMessage(session, chatId, messageId, translated.trim());
  }
}
...
// BOT_OWNER_PHONE (Render env var, OWNER_OVERRIDE here) names the owner by
// bare phone number. That's fine for a DM, but a group can present the SAME
// person tagged under a hidden @lid instead of their phone JID — and a
// LID's digits are an opaque id, not the phone number, so no amount of
// digit-comparison against OWNER_OVERRIDE will ever match one. The only way
// to know which @lid belongs to that phone number is to ask WAHA directly.
//
// This resolves OWNER_OVERRIDE into both forms — the phone-based JID and,
// where the engine tracks one, its @lid twin — ONCE per session, and saves
// them, so every later message's sender ids can be matched against a Set
// (getSelfIds/ownerIds) instead of falling back to scanning the group's
// whole participant list to work out who just spoke.
async function resolveOwnerOverrideIdentity(session) {
  if (!OWNER_OVERRIDE) return null;
  let jid = `${OWNER_OVERRIDE}@c.us`;
  let lid = '';
  try {
    const status = await checkNumberStatus(session, OWNER_OVERRIDE);
    const foundJid = jidFromValue(pickField(status, ['chatId', 'jid', 'id']));
    if (foundJid && !foundJid.endsWith('@lid')) jid = foundJid;
    const foundLid = jidFromValue(pickField(status, ['lid']));
    if (foundLid) lid = foundLid;
  } catch (error) {
    console.error('[owner] BOT_OWNER_PHONE check-exists lookup failed:', error?.response?.data || error?.message || error);
  }
  if (!lid) {
    try {
      const contact = await getContact(session, jid);
      const foundLid = jidFromValue(pickField(contact, ['lid']));
      if (foundLid) lid = foundLid;
    } catch {}
  }

  const stored = getStored(session);
  const identities = new Set([...(stored?.selfIdentities || []), jid]);
  if (lid) identities.add(lid);
  await upsertSession(session, {
    ownerOverridePhone: OWNER_OVERRIDE,
    ownerOverrideJid: jid,
    ownerOverrideLid: lid || null,
    ...(stored?.ownerJid ? {} : { ownerJid: jid }),
    selfIdentities: [...identities]
  });
  SELF_IDS_CACHE.delete(session);
  return { jid, lid };
}

// Fire-and-forget wrapper: kicks off the (network-bound) resolution above
// without making any caller wait on it, so recognizing the owner never adds
// latency to a command's response. Safe to call repeatedly — it's a no-op
// once a resolution for the CURRENT OWNER_OVERRIDE value is already stored
// or already in flight.
const OWNER_OVERRIDE_RESOLVING = new Set();
export function primeOwnerIdentity(session) {
  if (!OWNER_OVERRIDE || OWNER_OVERRIDE_RESOLVING.has(session)) return;
  const stored = getStored(session);
  if (stored?.ownerOverridePhone === OWNER_OVERRIDE && (stored?.ownerOverrideJid || stored?.ownerOverrideLid)) return;
  OWNER_OVERRIDE_RESOLVING.add(session);
  resolveOwnerOverrideIdentity(session)
    .catch((error) => console.error('[owner] override identity priming failed:', error?.message || error))
    .finally(() => OWNER_OVERRIDE_RESOLVING.delete(session));
}
...
async function getBotOwner(session) {
  ...
  if (OWNER_OVERRIDE) {
    const resolved = await resolveOwnerOverrideIdentity(session);
...
// Sends the exact uploaded repository media (image or video) mapped to a
// command. There is no channel lookup, remote media fetch, or media
...
async function sendCommandImage(session, chatId, key, caption) {
  ...
  return media.mime.startsWith('video/')
    ? sendVideo(session, chatId, file, caption)
    : sendImage(session, chatId, file, caption);
}
...
//
// "Know the linked account through phone number, JID or LID, any means
// possible" means pulling from every source that can possibly tell us:
//  1. Whatever WAHA's /me endpoint reports right now (phone JID and/or LID).
//  2. The single ownerJid captured once at pairing time.
//  3. Every id this account has ever been SEEN under (learnSelfIdentity
//     below) — the actual fix for id-mismatch cases: WhatsApp can present
//     the same account as a phone JID in one chat and an @lid in another,
//     and neither getMe() nor the pairing-time ownerJid is guaranteed to
//     cover every form it'll ever use.
//  4. An explicitly configured BOT_OWNER_PHONE, as a last-resort override.
...
async function getSelfIds(session) {
  ...
  try {
    for (const id of parseParticipant(await getMe(session)).ids) add(id);
  } catch {}
  add(await getBotOwner(session));
  const stored = getStored(session);
  for (const id of (stored?.selfIdentities || [])) add(id);
  if (OWNER_OVERRIDE) {
    add(`${OWNER_OVERRIDE}@c.us`);
    // Already-resolved jid/lid for BOT_OWNER_PHONE (no network call — just
    // reading what primeOwnerIdentity/resolveOwnerOverrideIdentity saved).
    if (stored?.ownerOverrideJid) add(stored.ownerOverrideJid);
    if (stored?.ownerOverrideLid) add(stored.ownerOverrideLid);
    primeOwnerIdentity(session); // background refresh if not resolved yet — never awaited here
  }
  SELF_IDS_CACHE.set(session, { ids, at: Date.now() });
  return ids;
}

// Passively grows the persisted set of known "this is the linked account"
// identifiers, so a mismatch (owner=false for a message that really is the
// owner) gets fixed permanently the next time it's actually seen, instead of
// depending on getMe() happening to report every id form up front.
//
// Deliberately restricted to GROUP messages with fromMe:true: participant/
// author there is exactly who typed, per WAHA. DM sender/from/author fields
// are NOT trustworthy for this even when fromMe is true — on some engines
// they describe the chat/other party rather than the sender (see the `from`
// resolution comment above) — and mislearning a stranger's id as "us" would
// be far worse than the mismatch this is meant to fix, so DMs are skipped.
async function learnSelfIdentity(session, chatId, payload) {
  if (!payload?.fromMe || !isGroup(chatId)) return;
  const info = payload?._data?.Info || {};
  const found = [payload.participant, payload.author, info.Sender, info.SenderAlt]
    .map(jidFromValue)
    .filter(Boolean);
  if (!found.length) return;
  const stored = getStored(session);
  const known = new Set(stored?.selfIdentities || []);
  let changed = false;
  for (const id of found) {
    if (!known.has(id)) { known.add(id); changed = true; }
  }
  if (!changed) return;
  await upsertSession(session, { selfIdentities: [...known] });
  SELF_IDS_CACHE.delete(session);
}
...
//
// Cached per-GROUP, not per-sender: the expensive part is the participant
// list fetch itself, which is the SAME data no matter who's asking. Caching
// it per (session, chat, sender) instead — as this used to — meant a busy
// group with many different people talking triggered a fresh full
// member-list fetch for EVERY distinct sender seen in each 15s window,
// instead of once per group. That's the actual cause of "the bot scans
// through the group / responds slowly" in large, active groups: deriving a
// specific sender's admin status from an already-fetched list is a cheap
// in-memory array scan, so only the fetch itself needs caching.
const GROUP_PARTICIPANTS_CACHE = new Map(); // `${session}:${chatId}` -> { at, entries, deep }
const GROUP_ADMIN_STATUS_TTL = Number(process.env.GROUP_ADMIN_CACHE_TTL_MS || 15000);

async function getGroupParticipantEntries(session, chatId, { needAdminFlags = false } = {}) {
  const key = `${session}:${chatId}`;
  const cached = GROUP_PARTICIPANTS_CACHE.get(key);
  const fresh = cached && Date.now() - cached.at < GROUP_ADMIN_STATUS_TTL;
  if (fresh && (!needAdminFlags || cached.deep)) return cached.entries;
  try {
    let entries = (await resolveParticipants(session, chatId, { deep: false })).map(parseParticipant);
    let deep = false;
    if (needAdminFlags && !entries.some((e) => e.isAdmin)) {
      try {
        const deepList = (await resolveParticipants(session, chatId, { deep: true })).map(parseParticipant);
        entries = mergeEntries(entries, deepList);
        deep = true;
      } catch {}
    }
    GROUP_PARTICIPANTS_CACHE.set(key, { at: Date.now(), entries, deep });
    return entries;
  } catch (error) {
...
async function getGroupAdminStatus(session, chatId, senderJid, payload) {
  ...
  const entries = await getGroupParticipantEntries(session, chatId, { needAdminFlags: true });
...
// Same repel treatment as sendAdminGateFailure, for the several owner-only
// sub-actions (.prefix-d add, .lang-add, etc.) that used to just reply with
// bare text — keeps every "you're not allowed to do that" reply looking and
// behaving the same way everywhere, never silent, never a plain one-liner.
function ownerOnlyDenial(session, chatId) {
  return sendAdminGateFailure(session, chatId, { repel: true, message: '❌ Owner only.' });
}
...
async function deleteWithWarning(session, chatId, messageId, senderJid, reason) {
  ...
  // Route through the shared warn/auto-kick system (see warnUser below) so
  // antilink/antichannel/kill-sale count toward the same configurable
  // warnLimit as badword/antispam, instead of being a delete-with-no-
  // consequence that a repeat offender can trigger forever.
  try { await warnUser(session, chatId, senderJid, reason); } catch {}
}
...
// Name-based tagging without a group scan: if `jid` is the CURRENT event's
// own sender, WAHA/WhatsApp already handed us their display name for free
// (notifyName / _data.Info.PushName) — zero extra calls. For any OTHER jid
// (e.g. tagging someone besides whoever triggered this), a single
// per-contact lookup (getContact) is used — NOT a group-member-list scan —
// with a short cache so the same jid doesn't get looked up repeatedly.
// Falls back to the phone-number format used everywhere else in the bot
// when no name is known (e.g. a @lid with no saved contact name).
const DISPLAY_NAME_CACHE = new Map(); // jid -> { name, at }
const DISPLAY_NAME_TTL = 10 * 60 * 1000;
async function resolveDisplayName(session, jid, payload) {
  const normalized = normalizeJid(jid) || jid;
  const eventSender = normalizeJid(payload?.from) === normalized || normalizeJid(payload?.participant) === normalized;
  if (eventSender) {
    const pushName = payload?.notifyName || payload?._data?.Info?.PushName || payload?._data?.pushName;
    if (pushName && String(pushName).trim()) return String(pushName).trim().slice(0, 60);
  }
  const cached = DISPLAY_NAME_CACHE.get(normalized);
  if (cached && Date.now() - cached.at < DISPLAY_NAME_TTL) return cached.name;
  let resolvedName = '';
  try {
    const contact = await getContact(session, normalized);
    resolvedName = String(contact?.name || contact?.pushname || contact?.notify || '').trim().slice(0, 60);
  } catch {}
  DISPLAY_NAME_CACHE.set(normalized, { name: resolvedName, at: Date.now() });
  return resolvedName;
}

// Builds the "@Name" (or "@1234567890" fallback) text for a mention, while
// the actual mentions[] array still carries the real jid — so WhatsApp
// clients that HAVE this contact saved still render their own name/tag
// correctly regardless of what text is here; this only controls what shows
// for clients that don't.
async function mentionLabel(session, jid, payload) {
  const name = await resolveDisplayName(session, jid, payload);
  return name ? `@${name}` : `@${jidNumber(jid)}`;
}
...
async function sendWelcomeOrGoodbye(session, groupId, participantJids, type, groupFromEvent = {}) {
  ...
  const memberLabels = await Promise.all(participantJids.map((jid) => mentionLabel(session, jid, {})));
  const memberText = memberLabels.join(', ');
...
async function warnUser(session, chatId, targetJid, reason) {
  ...
  const label = await mentionLabel(session, targetJid, {});
  if (count >= limit) {
    delete warns[targetJid];
    await setGroup(session, chatId, { warns });
    try { await removeParticipants(session, chatId, [targetJid]); } catch {}
    await sendText(session, chatId, `🚫 ${label} reached ${limit} warns and was removed.\n📝 ${reason}`, { mentions: [targetJid] });
  } else {
    warns[targetJid] = count;
    await setGroup(session, chatId, { warns });
    await sendText(session, chatId, `⚠️ ${label} warned (${count}/${limit})\n📝 ${reason}`, { mentions: [targetJid] });
  }
}
...
async function handleWebhookEventInternal(session, payload) {
  if (!session || !payload) return;
  let chatId = payload.from || payload.chatId || payload._data?.Info?.Chat;

  // WAHA's normalized `fromMe` flag isn't always reliable for every message
  // shape on every engine — GOWS in particular can leave it unset on some
  // extended/interactive messages — which is exactly what makes the linked
  // number's OWN typed commands sometimes get treated as a stranger's
  // message instead of the owner's. The raw engine-level `_data.Info.IsFromMe`
  // is the authoritative signal when present, so prefer it. Reassigning
  // payload.fromMe here (rather than threading a separate variable through
  // every fromMe check in this file) fixes owner recognition everywhere at
  // once, since all of them read this same field.
  const rawIsFromMe = payload?._data?.Info?.IsFromMe;
  if (typeof rawIsFromMe === 'boolean' && payload.fromMe !== rawIsFromMe) {
    payload.fromMe = rawIsFromMe;
  }

  // Reply to a DM using the phone JID when WhatsApp gave us an @lid chat id.
  // Sending to the phone twin is the most reliable path on GOWS.
  if (chatId && String(chatId).endsWith('@lid')) {
    const info = payload._data?.Info || {};
    const alts = payload.fromMe ? [info.RecipientAlt, info.SenderAlt] : [info.SenderAlt, info.RecipientAlt];
    const twin = alts.map(jidFromValue).find((j) => j && j.endsWith('@c.us'));
    if (twin) chatId = twin;
  }
  ...
  // GOWS sometimes leaves payload.body empty; fall back to the raw message.
  const rawMsg = payload._data?.Message || payload._data?.message || {};
  ...
  learnSelfIdentity(session, chatId, payload).catch(() => {}); // fire-and-forget
  ...
  // WhatsApp now delivers DMs from a hidden @lid id instead of the phone JID,
  // so compare EVERY id this sender carries (LID + phone twin) against EVERY id
  // the linked account has, rather than a single string equality.
  const selfIds = await getSelfIds(session);
  const senderIdsAll = senderIdsFromPayload(payload, from);
  // Non-@lid ids only: a LID's digits are an opaque identifier, not a phone
  // number, so comparing them by digit would be a coincidence, not a match.
  const selfPhoneDigits = new Set(selfIds.filter((id) => !id.endsWith('@lid')).map(jidNumber).filter(Boolean));
  const isOwner = Boolean(payload.fromMe)
    || senderIdsAll.some((id) => id === owner || selfIds.includes(id))
    || Boolean(OWNER_OVERRIDE && senderIdsAll.some((id) => jidNumber(id) === OWNER_OVERRIDE))
    // Last-resort: same phone number under formatting normalizeJid didn't catch.
    || senderIdsAll.some((id) => !id.endsWith('@lid') && selfPhoneDigits.has(jidNumber(id)));
  ...
  console.log(`[intake] ${session} chat=${isGroup(chatId) ? 'group' : 'dm'} fromMe=${Boolean(payload.fromMe)} owner=${isOwner} mode=${mode} bodyLen=${String(body).length} command=${parsed ? parsed.name : '-'}`);
  if (!parsed) {
    // The linked number's OWN messages (fromMe) can be genuinely EDITED in
    // place — WhatsApp only allows an account to edit a message IT sent —
    // so .lang-<language> on the linked number itself gets real AI
    // translation + an in-place edit, instead of the delete-and-resend
    // alphabet-substitution mode used for other members below. No .ai-style
    // extra round trips: one Gemini call, one edit, nothing else.
    if (payload?.fromMe && body.trim()) {
      const languageKey = getUserLanguage(session, chatId, from);
      const aiLanguageName = languageKey ? LANGUAGE_AI_NAMES[languageKey] : '';
      const id = payload?.id || payload?.messageId || payload?._data?.Info?.ID;
      if (languageKey && aiLanguageName && geminiConfigured() && id) {
        translateAndEdit(session, chatId, id, body, aiLanguageName)
          .catch((error) => console.error('[lang] translate+edit failed:', error?.message || error));
      } else if (languageKey && !aiLanguageName) {
        // A custom .lang-add cipher isn't a real language — AI can't
        // "translate" to a made-up character map, so keep the original
        // substitution behavior, just via edit now that fromMe allows it.
        const chars = getLanguageMap(languageKey);
        if (chars && id) {
          editMessage(session, chatId, id, mapEnglishAlphabet(body, chars)).catch(() => {});
        }
      }
      return;
    }
  ...
  if (!NO_INTRO_COMMANDS.has(parsed.name)) {
    // Fire-and-forget: the intro is decorative. Every command used to make
    // the person wait for THIS message to round-trip through WAHA before
    // even starting the real work — in a busy group that's a full extra
    // network hop of latency on every single command, for nothing.
    sendIntro(session, chatId).catch((error) => console.error('[bot] intro send failed:', error?.response?.data || error?.message || error));
  }
...
// Sends the main menu using the uploaded `21-menu.mp4` video and animates
// the caption in place. WAHA supports editing media captions, so the video
...
async function sendAnimatedMenuImage(session, chatId, text) {
  ...
  const send = media.mime.startsWith('video/') ? sendVideo : sendImage;

  let sent;
  try {
    sent = await send(session, chatId, file, frames[0]);
  } catch (error) {
    console.error('[menu] local media send failed:', error?.response?.data || error?.message || error);
...
async function executeCommand({ session, chatId, from, parsed, payload, isOwner }) {
  ...
  switch (name) {
    ...
    case 'prefix-d': {
      ...
      if (action === 'add') {
        if (!isOwner) return ownerOnlyDenial(session, chatId);
    ...
    case 'split-gc': {
      // Deliberately NOT assertGroupAdmin: reading a group's member list and
      // creating a brand-new group elsewhere needs no admin rights in the
      // SOURCE group — only being a member of it. Still owner-gated (isSelf)
      // so a random member can't trigger this.
      if (!isGroup(chatId)) return sendText(session, chatId, '❌ This command only works inside a group.');
      const status = await getGroupAdminStatus(session, chatId, from, payload);
      if (!status.isSelf) return sendAdminGateFailure(session, chatId, { repel: true, message: '⛔ Only the linked owner number can use admin features.' });

      // The member-adding loop can legitimately run for many minutes on a
      // large group (paced adds, by design). Awaiting it here would block
      // EVERY other command in this group for that whole time, since this
      // handler runs on the group's own command queue — so it's launched
      // detached instead, with its own progress/failure messages, and this
      // handler returns immediately.
      //
      // startSplitGc() checkpoints progress after every batch (see
      // advancedFeatures.js), so a redeploy or restart mid-job no longer
      // loses progress — resumeSplitGcJobs() picks it back up on the next
      // boot instead of it silently stopping after whatever batch was
      // in flight.
      (async () => {
        try {
          await sendCommandReply(session, chatId, '.split-gc', `🪞 Creating a new group and adding members ${SPLIT_GC_BATCH_SIZE || 5} at a time (paced, to avoid a WhatsApp spam flag). This can take a while for a large group — progress is saved as it goes, so a restart won't lose it. I'll post progress here.`);
          ...
          let lastReported = 0;
          const result = await startSplitGc(session, chatId, source, participants, botIds, {
            onProgress: async ({ added, total }) => {
              // Don't spam a message per batch of 5 — update roughly every 25
              // members (or on the final one).
              if (added - lastReported >= 25 || added >= total) {
                lastReported = added;
                await sendText(session, chatId, `🪞 Split group progress: ${added}/${total} members added...`).catch(() => {});
              }
            }
          });
          ...
          await sendText(session, chatId, `❌ Could not create the clone: ${String(error?.response?.data?.message || error?.message || error).slice(0, 260)}`).catch(() => {});
        }
      })();
      return;
    }
...
async function moderateIncoming(session, chatId, payload, senderJid, body, { fastOnly = false } = {}) {
  ...
  if (config.antibot && !payload?.fromMe) {
    ...
    if (shouldDelete) {
      if (knownBot) {
        // A confirmed bot account (on ANTIBOT_JIDS) gets no grace period —
        // delete + remove immediately, bypassing the warn counter entirely.
        if (messageId) { try { await deleteMessage(session, chatId, messageId); } catch {} }
        try { await sendText(session, chatId, `🤖 @${jidNumber(senderJid)} — bot account detected, message removed.`, { mentions: [senderJid] }); } catch {}
        if (ANTIBOT_KICK) {
        ...
        // Heuristic-only match (menu-shaped text, bot vocabulary, etc.) — give
        // the benefit of the doubt via the shared warn/auto-kick system rather
        // than an instant, unappealable delete on a possible false positive.
        await deleteWithWarning(session, chatId, messageId, senderJid, `🤖 Bot-style response detected (${signal.score}/${ANTIBOT_SCORE_THRESHOLD}).`);
      }
      return true;
    }
  }
