  ...
  if (type === 'demote' && config.antidemote) {
    for (const target of targets) {
      ...
      try {
        ...
        markSelfAction(groupId, target, 'promote');
        if (canonicalTarget !== target) markSelfAction(groupId, canonicalTarget, 'promote');
  ...
  if (type === 'promote' && config.antipromote) {
    for (const target of targets) {
      ...
      try {
        ...
        markSelfAction(groupId, target, 'demote');
        if (canonicalTarget !== target) markSelfAction(groupId, canonicalTarget, 'demote');
...
async function sendMenuMessage(session, chatId, prefix) {
  ...
  if (interactiveSendBroken) return;
  try {
...
async function executeCommand({ session, chatId, from, parsed, payload, isOwner }) {
  ...
  switch (name) {
    ...
    case 'link': {
      ...
      const channelUrl = String(process.env.ORIHIME_CHANNEL_LINK || '').trim();
      const lines = [
        '╭━━━〔 🔗 𝐋𝐈𝐍𝐊𝐒 〕━━━╮',
        ...(channelUrl ? ['┃ 📡 𝐎𝐟𝐟𝐢𝐜𝐢𝐚𝐥 𝐂𝐡𝐚𝐧𝐧𝐞𝐥', `┃ ${channelUrl}`] : []),
        '┃',
        '┃ 🤖 𝐂𝐡𝐚𝐭 𝐰𝐢𝐭𝐡 𝐭𝐡𝐞 𝐁𝐨𝐭',
    ...
    case 'autod': {
      const gate = await assertGroupAdmin(session, chatId, from, payload);
      if (!gate.ok) return sendText(session, chatId, gate.message);
      const target = (await resolveCommandTargets(session, chatId, payload, args))[0];
