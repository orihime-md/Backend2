...
const RAPIDAPI_HOST = (process.env.RAPIDAPI_YOUTUBE_HOST || 'youtube-media-downloader.p.rapidapi.com').trim();
const RAPIDAPI_BASE = `https://${RAPIDAPI_HOST}`;
const SEARCH_PATH = process.env.RAPIDAPI_YOUTUBE_SEARCH_PATH || '/v2/search/videos';
const DOWNLOAD_PATH = process.env.RAPIDAPI_YOUTUBE_DOWNLOAD_PATH || '/v2/video/details';
...
async function rapidGet(path, params) {
  ...
  try {
    ...
    if (response.status < 200 || response.status >= 300) {
      ...
      const error = new Error(`RapidAPI HTTP ${response.status}: ${detail.slice(0, 220)}`);
      error.status = response.status;
      error.rapidApiDetail = detail.slice(0, 500);
      throw error;
    }
    console.log('[rapidapi] request ok', { path, status: response.status, durationMs: Date.now() - startedAt });
    return response.data;
  } catch (error) {
    const status = error?.status || error?.response?.status || null;
    const detail = error?.rapidApiDetail
    ...
    console.error('[rapidapi] request failed', {
      ...
      params: params || {},
      detail: String(detail || error?.message || error).slice(0, 500)
    });
    if (error?.status) throw error;
    if (error?.response?.status) {
      const wrapped = new Error(`RapidAPI HTTP ${error.response.status}: ${String(detail || '').slice(0, 220)}`);
      wrapped.status = error.response.status;
      wrapped.rapidApiDetail = String(detail || '').slice(0, 500);
      throw wrapped;
    }
    throw error;
  }
...
export async function rapidYoutubeSearch(query) {
  const searchParam = String(process.env.RAPIDAPI_YOUTUBE_SEARCH_PARAM || 'keyword').trim() || 'keyword';
  const data = await rapidGet(SEARCH_PATH, { [searchParam]: query.trim() });
...
export async function rapidYoutubeDownload(videoId) {
  const downloadParam = String(process.env.RAPIDAPI_YOUTUBE_DOWNLOAD_PARAM || 'videoId').trim() || 'videoId';
  const data = await rapidGet(DOWNLOAD_PATH, { [downloadParam]: videoId });
  ...
  let response;
  try {
    response = await axios.get(audio.url, {
    ...
    console.error('[rapidapi] audio download failed', {
      videoId,
  ...
  console.log('[rapidapi] audio download ok', {
    videoId,
