...
// Session-scoped guard: used on routes that manage ONE linked WhatsApp account.
// Access is granted to either the master DASHBOARD_TOKEN (support/admin) or the
// token that was generated for that specific session when it was created. This
// is what makes self-service pairing possible — a visitor never needs your
// admin token, only the token their own session gave them back.
function guardSession(req, res, next) {
  const master = process.env.DASHBOARD_TOKEN;
  const supplied = req.get('x-dashboard-token') || req.query.token;
  if (master && supplied && supplied === master) return next();

  const session = req.params.session;
  const stored = getStored(session);
  if (!stored || stored.removed) return res.status(404).json({ error: 'Session not found' });
  if (!stored.token || supplied !== stored.token) {
    return res.status(401).json({ error: 'Unauthorized — this session token does not match.' });
  }
  next();
}
...
app.post('/api/link', async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({
      session: name,
      // Every self-service caller gets their own token back here — this is the
      // only credential they need for /status, /pairing-code and /remove on
      // THIS session. Save it (the dashboard keeps it visible too).
      token,
...
app.get('/api/link/:session/status', guardSession, async (req, res) => {
  const session = req.params.session;
  const stored = getStored(session);
  if (!stored || stored.removed) return res.status(404).json({ error: 'Session not found' });
  try {
...
app.post('/api/link/:session/pairing-code', guardSession, async (req, res) => {
  const session = req.params.session;
  const stored = getStored(session);
  if (!stored || stored.removed) return res.status(404).json({ error: 'Session not found' });
  const phoneNumber = normalizePhoneNumber(req.body?.phoneNumber);
...
app.post('/api/link/:session/remove', guardSession, async (req, res) => {
  const session = req.params.session;
  const stored = getStored(session);
