...
import { handleWebhookEvent, handleGroupEvent } from './lib/bot.js';
import { ingestChannelEvent, syncChannelImages } from './lib/channelSync.js';
...
async function waitForPairingReady(name, timeoutMs = 20000) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    try {
      ...
      if (status === 'SCAN_QR_CODE' || status === 'WORKING' || status === 'FAILED' || status === 'STOPPED') return last;
    } catch {}
...
app.post('/api/link/:session/pairing-code', guardDashboard, async (req, res) => {
  ...
  try {
    const remote = await getSession(session);
    if (String(remote?.status || '').toUpperCase() === 'WORKING') {
