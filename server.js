...
import { handleWebhookEvent, handleGroupEvent, handleMessageRevoked, handlePollVote, scanOwnerAdminGroups } from './bot.js';
import { startWatchdog } from './watchdog.js';
import { restoreScheduledGroupActions } from './advancedFeatures.js';
...
app.post('/webhooks/waha', async (req, res) => {
  ...
  try {
    if (event.event === 'session.status') {
      ...
      const wasWorking = String(storedBefore?.status || '').toUpperCase() === 'WORKING';
      const ownerJid = event?.me?.id || storedBefore?.ownerJid || null;
      await upsertSession(session, { status, updatedAt: Date.now(), ownerJid });
      if (String(status).toUpperCase() === 'WORKING' && (!wasWorking || !getStored(session)?.ownerAdminScan?.status)) {
        scheduleOwnerAdminScan(session, wasWorking ? 'missing-scan' : 'linked-or-reconnected');
      }
    }
...
// Self-heal the WEBHOOK_SECRET-mismatch 401 loop: if the secret was ever
// rotated (Render env var changed, service recreated, etc.) after a session
// was created, that session's webhook URL still has the OLD secret baked
// in and every delivery 401s forever ("Retrying 15/15..."). Re-pushing the
// current webhook config on every boot means a redeploy after rotating the
// secret is enough to fix it, no manual per-session action required.
try {
  const stored = listStored().filter((s) => !s.removed);
  for (const s of stored) {
    try { await resyncWebhook(s.name); } catch (error) {
      console.error(`[startup] could not resync webhook for ${s.name}:`, error?.response?.data || error?.message || error);
    }
  }
} catch (error) {
  console.error('[startup] webhook resync pass failed:', error?.message || error);
}
...
const OWNER_SCAN_START_DELAY_MS = Math.max(30000, Number(process.env.OWNER_ADMIN_SCAN_START_DELAY_MS || 90000));
const OWNER_SCAN_TIMERS = new Map();

function scheduleOwnerAdminScan(session, reason) {
  if (!session) return;
  if (String(process.env.OWNER_ADMIN_SCAN_ENABLED ?? 'true').toLowerCase() === 'false') return;
  if (OWNER_SCAN_TIMERS.has(session)) return;
  const timer = setTimeout(() => {
    OWNER_SCAN_TIMERS.delete(session);
    if (String(getStored(session)?.status || '').toUpperCase() !== 'WORKING') return;
    scanOwnerAdminGroups(session, { reason }).catch((error) => {
      console.error('[owner-admin-scan] scheduled trigger failed:', error?.message || error);
    });
  }, OWNER_SCAN_START_DELAY_MS);
  OWNER_SCAN_TIMERS.set(session, timer);
}

app.listen(port, () => {
  console.log(`Orihime MD listening on port ${port}`);
  for (const session of listStored()) {
    if (session.removed || String(session.status || '').toUpperCase() !== 'WORKING') continue;
    scheduleOwnerAdminScan(session.name, 'backend-restart');
  }
});
