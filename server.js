...
try {
  ...
  for (const s of list) {
    ...
    // Push the current webhook config (events list, URL, secret) to every
    // existing WAHA session on every boot. WAHA persists a session's webhook
    // config server-side from whenever it was created/last synced, so a code
    // change to buildWebhookConfig() (e.g. adding an event) never reaches an
    // already-registered session on its own — this is what applies it,
    // without needing someone to remember to hit the admin resync button.
    resyncWebhook(s.name).catch((error) => console.error(`[startup] webhook resync failed for ${s.name}:`, error?.message || error));
  }
} catch (error) {
