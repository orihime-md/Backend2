  ...
  try {
    if (event.event === 'session.status') {
      ...
      if (String(status).toUpperCase() === 'WORKING' && (!wasWorking || !getStored(session)?.ownerAdminScan?.status)) {
        setTimeout(() => {
          scanOwnerAdminGroups(session, { reason: wasWorking ? 'missing-scan' : 'linked-or-reconnected' }).catch((error) => {
            console.error('[owner-admin-scan] status trigger failed:', error?.message || error);
          });
        }, 1500);
      }
    }
...
app.listen(port, () => {
  ...
  setTimeout(() => {
    for (const session of listStored()) {
      ...
      scanOwnerAdminGroups(session.name, { reason: 'backend-restart' }).catch((error) => {
        console.error('[owner-admin-scan] startup trigger failed:', error?.message || error);
      });
    }
  }, 1500);
});
